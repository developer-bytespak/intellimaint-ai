...
export function useChat() {
  ...
  const [hasReceivedFirstToken, setHasReceivedFirstToken] = useState<{
  ...
  const uploadImages = useCallback(
    async (imageUrls: string[]): Promise<string[]> => {
      ...
      if (isUserLoadingRef.current || !userRef.current) {
        ...
        while (
          (isUserLoadingRef.current || !userRef.current) &&
      ...
      if (!currentUser?.id) {
        ...
        throw new Error(
          "User ID not available. Please ensure you are logged in and try again."
        );
      }
  ...
  useEffect(() => {
    const loadSessions = async () => {
      try {
        ...
        const isPageReload =
          sessionStorage.getItem("chatPageReloaded") === "true";
  ...
  useEffect(() => {
    ...
    if (chatId && chatId !== "") {
      ...
      if (
        currentActiveChat &&
        (!currentActiveChat.id || currentActiveChat.id === "")
      ) {
        ...
        try {
          ...
          setChats((prev) => {
            ...
            if (exists) {
              ...
              return prev.map((c) =>
                c.id === fetchedChat.id ? fetchedChat : c
              );
            } else {
  ...
  const createNewChat = useCallback(
    (skipRedirect: boolean = false): Chat => {
      ...
      setChats((prev) =>
        prev.filter((chat) => chat.id && chat.messages.length > 0)
      );
  ...
  const selectChat = useCallback(
    async (chat: Chat) => {
      try {
        ...
        setChats((prev) =>
          prev.filter((c) => (c.id && c.id !== "") || c.id === chat.id)
        );
  ...
  const sendMessage = useCallback(
    async (
      ...
      if (isUserLoading || !user || !user.id) {
        console.error("❌ Cannot send message: User not loaded yet", {
          isUserLoading,
          hasUser: !!user,
          hasUserId: !!user?.id,
        });
      ...
      try {
        ...
        if (editingMessageIdParam && chatToUse) {
          const messageIndex = chatToUse.messages.findIndex(
            (m) => m.id === editingMessageIdParam
          );
          if (messageIndex !== -1) {
            ...
            if (
              chatToUse.id &&
              messageToEdit.id &&
              !messageToEdit.id.startsWith("temp-")
            ) {
              try {
        ...
        if (editingMessageIdParam) {
          ...
          optimisticChat = {
            ...
            updatedAt: new Date(),
            title:
              finalChatToUse.title === "New Chat" &&
              finalChatToUse.messages.length === 0
                ? content.length > 50
        ...
        try {
          ...
          const finalText =
            ...
            streamingText[tempAssistantMessageId] ||
            "";
          ...
          try {
            ...
            const realMessage = completeChat.messages.find(
              (msg) =>
                msg.role === "assistant" &&
            ...
            if (wasStopped) {
              completeChat.messages = completeChat.messages.map((msg, idx) => {
                ...
                if (
                  msg.role === "user" &&
                  idx === completeChat.messages.length - 2
                ) {
                  return { ...msg, isStopped: true };
                }
                return msg;
        ...
        if (chatToUse) {
          setChats((prev) =>
            prev.map((chat) => (chat.id === chatToUse.id ? chatToUse : chat))
          );
  ...
  const searchingofSpecificChat = useCallback(
    (chatId: string) => {
  ...
  const stopStreaming = useCallback(async () => {
    ...
    if (activeChat) {
      ...
      try {
        ...
        console.log(
          "stopStreaming: updated messages",
