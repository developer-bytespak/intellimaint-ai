...
function PricingCard({ tier, index }: { tier: PricingTier; index: number }) {
  ...
  return (
    <div ref={cardRef} className="relative h-full">
      {tier.popular && (
        <div className="absolute -top-2.5 left-1/2 -translate-x-1/2 z-10"><span className="inline-block bg-[#fc4f02] text-white text-[10px] font-semibold px-2.5 py-0.5 rounded uppercase tracking-wide">Popular</span></div>
      )}

      <div className={`relative rounded-lg border-2 bg-gradient-to-br from-[--color-surface-alt]/90 via-[--color-surface-alt]/70 to-[--color-surface-alt]/90 backdrop-blur-xl p-5 h-full flex flex-col transition-all duration-200 ${tier.popular ? "border-[#fc4f02]/60" : "border-[--color-border]"} ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"} hover:border-[#fc4f02]/60`} style={{ transitionDelay: isVisible ? `${index * 100}ms` : "0ms" }}>
        <div className="flex flex-col h-full">
          ...
          <button onClick={handleGetStarted} disabled={isCheckingAuth} className={`w-full rounded-md px-4 py-2.5 text-xs font-semibold transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${tier.popular ? "bg-[#fc4f02] text-white hover:bg-[#e04502]" : "border border-[--color-border] bg-[--color-surface] text-white hover:border-[#fc4f02]/50 hover:bg-[--color-surface-alt]"}`}>{isCheckingAuth ? "Checking..." : tier.price === "Custom" ? "Contact Sales" : "Get Started"}</button>
        </div>
      </div>
    </div>
  );
...
export function PricingSection() {
  const tiers: PricingTier[] = [
    { name: "Free", price: "$0", period: "month", description: "Perfect for getting started", features: ["Basic AI trading strategies", "Real-time market data", "Single exchange connection", "Community support", "Basic portfolio tracking"], gradient: "from-slate-600 to-slate-700" },
    { name: "Pro", price: "$99", period: "month", description: "For serious traders", popular: true, features: ["Advanced AI strategies", "Real-time sentiment analysis", "Multi-exchange connectivity", "Portfolio optimization", "Priority support", "Advanced analytics"], gradient: "from-[#fc4f02] to-[#fda300]" },
    ...
    <section id="pricing" className="relative pb-20 sm:pb-24 lg:pb-32">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mt-16">
          ...
          <button onClick={() => { const element = document.getElementById("contact"); if (element) element.scrollIntoView({ behavior: "smooth" }); }} className="text-[#fc4f02] hover:text-[#e04502] font-semibold transition-colors cursor-pointer text-sm">Contact Sales →</button>
        </div>
      </div>
    </section>
  );
