...
export default function RepositoryPage() {
  ...
  const { uploadDocument, deleteDocument, extractDocument, getFileForJob, clearFileForJob, storeFileByFileId, getFileByFileId, cancelExtraction } = useRepository()
  ...
  const unsubscribe = queryClient.getQueryCache().subscribe((event) => {
    if (event?.query?.queryKey?.[0] === 'extraction-progress') {
      ...
      // Sirf tab update karein agar status ya progress waqai badli ho
      if (newData?.status !== extractionProgress?.status || newData?.progress !== extractionProgress?.progress) {
        if (typeof window !== 'undefined') {
  ...
  // Is logic ko component ki body se nikaal kar useEffect mein dalein
  useEffect(() => {
    if (extractionProgress?.status === "completed") {
      console.log("Extraction Completed for Job:", activeJobId);
      // Yahan aap mazeed logic add kar sakte hain jo sirf ek baar chalega
    }
  }, [extractionProgress?.status]); // Sirf status change hone par chalega
  ...
  const removeSelectedFile = async (id: string) => {
    // Find if this file is currently being processed
    const fileInQueue = processingQueue.find(item => item.id === id)
    const isCurrentlyProcessing = fileInQueue && activeJobId
    
    // If this is the currently processing file, cancel the extraction
    if (isCurrentlyProcessing && activeJobId) {
      try {
        // Cancel the background processing
        await cancelExtraction(activeJobId)
        ...
        // Clear session storage for this job
        if (typeof window !== 'undefined') {
          sessionStorage.removeItem(STORAGE_KEY)
          sessionStorage.removeItem(`${STORAGE_KEY}-filename`)
          sessionStorage.removeItem(`${STORAGE_KEY}-uploadTriggered`)
        ...
        // Clear the file from global storage
        ...
        setTimeout(() => {
          setProcessingQueue(prev => {
            const newQueue = prev.slice(1) // Remove current file
            // Update queue in sessionStorage
            if (typeof window !== 'undefined') {
              if (newQueue.length > 0) {
                const queueMetadata = newQueue.map(f => ({ id: f.id, name: f.name, size: f.size }))
    ...
    // Remove from queue if present (for files not currently processing)
    setProcessingQueue(prev => {
      const newQueue = prev.filter(item => item.id !== id)
      // Update queue metadata in sessionStorage
      if (typeof window !== 'undefined') {
        if (newQueue.length > 0) {
          const queueMetadata = newQueue.map(f => ({ id: f.id, name: f.name, size: f.size }))
          sessionStorage.setItem(STORAGE_QUEUE_KEY, JSON.stringify(queueMetadata))
    ...
    if (typeof window !== 'undefined') {
      try {
        sessionStorage.removeItem(`file-queue-${id}`)
      } catch (e) {
        console.error('Failed to clear file from storage:', e)
      }
    }
    
    // Update selected files metadata in sessionStorage
    if (typeof window !== 'undefined') {
      setTimeout(() => {
        const currentSelectedFiles = JSON.parse(sessionStorage.getItem(STORAGE_SELECTED_FILES_KEY) || '[]')
        const updatedFiles = currentSelectedFiles.filter((file: any) => file.id !== id)
        if (updatedFiles.length > 0) {
          sessionStorage.setItem(STORAGE_SELECTED_FILES_KEY, JSON.stringify(updatedFiles))
        } else {
          sessionStorage.removeItem(STORAGE_SELECTED_FILES_KEY)
        }
      }, 100)
    }
  }
